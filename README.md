...
### 🌐 智能翻译
- **Azure OpenAI 大模型** - 使用大模型进行智能翻译，理解上下文更准确
...
### 🎯 交互体验
- **行内 CodeLens 按钮** - 选中文本后，在代码行上方直接显示 🌐 翻译 和 💡 命名 按钮，点击即执行
- **快捷键支持** - `Alt+T` 翻译、`Alt+N` 命名，无需鼠标操作
- **右键菜单** - 传统右键菜单方式，适合习惯右键操作的用户
- **结果面板** - 极简风格的翻译结果展示，支持复制和替换
- **配置灵活** - 可自定义 API Key、目标语言、建议数量、是否显示行内按钮等
...
- **Enable Quick Actions** - 启用行内快速操作按钮（默认: true，选中文本时在行上方显示 🌐 翻译 和 💡 命名 按钮）
...
**方式一：行内 CodeLens 按钮（推荐）**
1. 选中文本
2. 在选中的代码行上方自动出现 🌐 翻译 按钮
3. 点击按钮即可翻译
...
│   │   └── naming.ts             # 命名命令
│   ├── providers/
│   │   ├── azureOpenAIService.ts # Azure OpenAI 服务
│   │   └── codeLensProvider.ts   # 行内 CodeLens 按钮提供者
│   ├── ui/
│   │   └── resultPanel.ts        # 结果面板（极简风格）
│   └── utils/
...
- 支持 AI 生成变量命名建议
- **行内 CodeLens 按钮** - 选中文本后，在代码行上方直接显示 🌐 翻译 和 💡 命名 按钮，点击即执行
- **配置项** - `translateHelper.enableQuickActions` 控制是否显示行内按钮
- **极简风格 UI** - 结果面板使用 VS Code 原生主题色，简洁清爽
- 支持快捷键（Alt+T / Alt+N）和右键菜单
- 支持多种编程语言的命名风格
