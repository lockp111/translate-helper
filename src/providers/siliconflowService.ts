 ...
 */
export class SiliconFlowService implements ILLMProvider {
    ...
    private initializeClient(): boolean {
        ...
        try {
            this.client = new OpenAI({
                apiKey: apiKey,
                baseURL: SiliconFlowService.BASE_URL
            });
            return true;
        } catch (error) {
            console.error('Failed to initialize SiliconFlow client:', error);
            this.client = null;
            return false;
        }
    }
    ...
    async translateStream(text: string, callbacks: StreamCallbacks): Promise<void> {
        if (!this.client) {
            if (!this.initializeClient()) {
                throw new Error('请配置硅基流动 API Key');
            }
        }
