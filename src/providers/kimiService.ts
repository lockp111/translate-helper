    ...
    async translateStream(text: string, callbacks: StreamCallbacks): Promise<void> {
        if (!this.client) {
            if (!this.initializeClient()) {
                throw new Error('请配置 Kimi API Key');
            }
        }

        const config = vscode.workspace.getConfiguration('translateHelper');
        const targetLang = config.get<string>('targetLanguage', 'zh');
        const model = config.get<string>('kimiModel')!;

        try {
            // @ts-ignore - Kimi specific parameter to disable thinking mode
            const response = await this.client!.chat.completions.create({
                ...
                stream: false,
                // @ts-ignore - Kimi specific: disable thinking mode for faster response
